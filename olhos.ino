...
#define EYE_1_XPOSITION  10         // x shift for eye 1 image on display
#define EYE_2_XPOSITION  370 - 28 // x shift for eye 2 image on display
...
#endif
#if !defined(IRIS_MAX)
  #define IRIS_MAX      628 // Iris size (0-1023) in darkest light
#endif
...
void frame(uint16_t iScale) // Iris scale (0-1023)
{
  ...
  if (tft.getTouchRawZ() > 500) {
                ...
                tft.setTextColor(TFT_RED, TFT_BLACK); // Change the font colour and the background colour
                tft.println("FIM");
                tft.setCursor(180, 230);  // Posição do cursor na tela
                tft.setTextSize(1);
                tft.setTextColor(TFT_GREEN, TFT_BLACK); // Change the font colour and the background colour
